    ...
    <div class="steps-content">
      <div v-if="current === 0">
        <a-form-model :model="form" :label-col="labelCol" :wrapper-col="wrapperCol">
          <a-form-model-item label="寄件人" required>
            <a-input v-model="form.company"/>
          </a-form-model-item>
          <a-form-model-item label="付款" required>
            <a-input v-model="form.number"/>
          </a-form-model-item>
          <a-form-model-item label="货物类型" required>
            <a-select v-model="selectIndex" placeholder="请选择货物类型">
      ...
      <div v-if="current === 1" class="check">
        <p>收件人： {{ form.company }}</p>
        <p>收款账号： {{ form.number }}</p>
        <p>售出货物： {{ form.commodity }}</p>
        <p>货物数量： {{ form.count }}</p>
        <p>预留电话： {{ form.phone }}</p>
